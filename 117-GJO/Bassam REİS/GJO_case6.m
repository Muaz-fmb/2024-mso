function [bestSolution, bestFitness, iteration]=GJO_case6(fhd, dimension, maxIteration, fNumber)
    ...
    for i=1:size(Positions,1)
        for j=1:size(Positions,2)     
           ...
           E=E1*E0; % Evading energy
            ...
            %% EXPLORATION
            % Randomly pick a position from the current population
            randIndex = randi(size(Positions, 1)); % Random index
            randomPosition = Positions(randIndex, :); % Randomly picked position
            
            D_male_jackal = abs((Male_Jackal_pos(j) - RL(i, j) * randomPosition(j)));
            Male_Positions(i, j) = Male_Jackal_pos(j) - E * D_male_jackal;
            D_female_jackal = abs((Female_Jackal_pos(j) - RL(i, j) * randomPosition(j)));
            Female_Positions(i, j) = Female_Jackal_pos(j) - E * D_female_jackal;
             end
