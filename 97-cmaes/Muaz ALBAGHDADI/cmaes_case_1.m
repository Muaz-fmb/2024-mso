function [bestSolution, bestFitness, iteration]=cmaes_case_1(fhd, dimension, maxIteration, fNumber)
...
for g = 1:MaxIt
    ...
    % Sort Population Costs (Optional for clarity)
    Costs = [pop.Cost];
    [~, SortOrder] = sort(Costs); % Optional sorting for clarity
    pop = pop(SortOrder);        % Optional sorting for clarity

    % Normalize Fitness Values
    minFitness = min(Costs);
    maxFitness = max(Costs);
    normalizedFitness = (Costs - minFitness) ./ (maxFitness - minFitness);

    % Roulette Wheel Selection
    cumulativeSum = cumsum(normalizedFitness);
    selectionValue = rand * cumulativeSum(end);
    [~, selectedIndex] = find(cumulativeSum >= selectionValue, 1);
    selectedIndividual = pop(selectedIndex, :);
    ...
    M(g+1).Step = M(g+1).Step + selectedIndividual.Step; 
