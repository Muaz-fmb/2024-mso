function [bestSolution, bestFitness, iteration]=cmaes_case_2(fhd, dimension, maxIteration, fNumber)
...
for g = 1:MaxIt
    ...
    % Sort Population
    % Check for Empty Population (after sample generation)
    if isempty([pop.Position]) 
        % Handle Empty Population (use previous best solution)
        selectedIndividual = BestSol;  % Use the previous best solution
    else
        % Sort Population
        Costs = [pop.Cost];
        ...
        % Fitness-Distance Balance Selection (FDB)
        selectedIndividual = fitnessDistanceBalance([pop.Position], [pop.Cost]);
    end
